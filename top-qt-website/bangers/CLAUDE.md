...
## Strand Atlas Data Generation

The detailed strand atlas visualization uses data generated by `scratchpads/22_generate_atlas_data.py`:

```bash
cd scratchpads && uv run python 22_generate_atlas_data.py
```

This script:
1. Loads UMAP-projected embeddings from `data/tweet_embeddings_atlas.parquet`
2. Loads rated strands from `data/rated_strands/` for metadata
3. Loads `tweet_dict.diskcache` for proper usernames
4. Outputs `public/atlas_data.json` for the frontend

The atlas supports click-to-highlight: clicking a node highlights that strand's root + essential tweets while dimming others.
